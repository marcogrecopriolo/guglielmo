...
# hack to produce a populated appimage even on newer glibc's
glibc=`ldd --version | awk '{ print $4*100; exit }'`
if test $glibc -gt 235
then
    mkdir -p appdir/usr/share/doc/libc6/copyright
    extra="-unsupported-allow-new-glibc"
fi
...
make -j4
cd ..
mkdir -p appdir/usr/bin appdir/usr/lib appdir/usr/plugins appdir/usr/share/applications appdir/usr/share/icons/hicolor/256x256/apps
...
./linuxdeployqt-continuous-${arch}.AppImage ./appdir/usr/share/applications/* -verbose=2 $extra -bundle-non-qt-libs -no-translations -extra-plugins=styles
