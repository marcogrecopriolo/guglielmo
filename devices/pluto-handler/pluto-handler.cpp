...
static QString get_ch_name(QString type, int id) {
    QString result = type;
...
plutoHandler::plutoHandler()
    ...
    chn = iio_device_find_channel(phys_dev,
        get_ch_name(QString("voltage"), 0).toLatin1().data(),
        false);
    if (chn == nullptr) {
    ...
    this->lo_channel = iio_device_find_channel(phys_dev,
...
int32_t plutoHandler::getSamples(std::complex<float>* V, int32_t size, agcStats* stats) {
    if (!isRunning())
        return 0;
