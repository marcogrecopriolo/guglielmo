...
void	myGainChangeCallback (uint32_t	gRdB,
	                      uint32_t	lnaGRdB,
	                      void	*cbContext) {
//	log (DEV_PLAY, LOG_MIN, "GainChangeCallback gives %X", gRdB);
	(void)gRdB;
	(void)lnaGRdB;	
	(void)cbContext;
}
...
int32_t	sdrplayHandler::getSamples (std::complex<float> *V, int32_t size, int32_t *gainChange) { 
	*gainChange = 0;
