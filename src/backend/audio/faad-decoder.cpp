...
faadDecoder::~faadDecoder() { NeAACDecClose(aacHandle); }
...
int16_t faadDecoder::MP42PCM(stream_parms *sp, uint8_t buffer[],
    ...
    if ((sampleRate == 24000) || (sampleRate == 32000) ||
        (sampleRate == 48000) || (sampleRate != (long unsigned)baudRate))
        baudRate = sampleRate;

    log(LOG_AUDIO, LOG_VERBOSE,
        "header %d bytes consumed %d samplerate %ld, samples %d, channels %d, "
        "error %d, sbr %d",
        hInfo.header_type, (int)(hInfo.bytesconsumed), sampleRate, samples,
        hInfo.channels, hInfo.error, hInfo.sbr);
    channels = hInfo.channels;
    if (hInfo.error != 0) {
        log(LOG_AUDIO, LOG_MIN, "Warning: %s",
