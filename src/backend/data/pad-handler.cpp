...
void padHandler::processPAD(uint8_t *buffer, int16_t last, uint8_t L1,
                            uint8_t L0) {
    ...
    if (CIf != 0) { // has a CI flag
        ...
        case 3: // continuation of fragment
            ...
            if ((still_to_go <= 0) && (shortpadData.size() > 1)) {
                ...
                dynamicLabelText.append(toQStringUsingCharset(
                    (char *)(shortpadData.data()), (CharacterSet)charSet,
                    shortpadData.size()));
                shortpadData.resize(0);
            }
            break;
        ...
        if ((still_to_go <= 0) && (shortpadData.size() > 0)) {
            ...
            dynamicLabelText.append(toQStringUsingCharset(
                (char *)(shortpadData.data()), (CharacterSet)charSet));
            shortpadData.resize(0);
            //	if we are at the end of the last segment (and the text is not
            //empty) 	then show it.
            if (!firstSegment && lastSegment) {
                if (dynamicLabelText.size() > 0)
                    showText(dynamicLabelText);
...
void padHandler::handle_variablePAD(uint8_t *b, int16_t last, uint8_t CI_flag) {
    int16_t CI_Index = 0;
    uint8_t CI_table[4];
    int16_t i, j;
    int16_t base = last;
    ...
    for (i = 0; i < CI_Index; i++) {
        ...
        if (base < 0 && i < CI_Index - 1) {
            log(LOG_DATA, LOG_MIN, "invalid base %d or index %d", base,
                CI_Index);
            return;
        }
    }
    ...
    if ((CI & 037) == 02) { // start of segment
        ...
        if (Cflag) { // special dynamic label command
            ...
            QString segmentText = toQStringUsingCharset(
                (const char *)&data[2], (CharacterSet)charSet, dataLength);
...
void padHandler::new_MSC_element(std::vector<uint8_t> data) {
    ...
    if (data.size() >=
        (uint16_t)dataGroupLength) { // msc element is single item
        msc_dataGroupBuffer.clear();
        build_MSC_segment(data);
        mscGroupElement = false;
        show_motHandling(true);
...
void padHandler::add_MSC_element(std::vector<uint8_t> data) {
    ...
    msc_dataGroupBuffer.insert(std::end(msc_dataGroupBuffer), std::begin(data),
                               std::end(data));
...
void padHandler::build_MSC_segment(std::vector<uint8_t> data) {
    ...
    //	is
    int32_t size =
        data.size() < (uint32_t)dataGroupLength ? data.size() : dataGroupLength;
