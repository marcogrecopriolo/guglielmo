...
dabProcessor::dabProcessor(RadioInterface *mr, deviceHandler *inputDevice,
    ...
    totalFrames = 0;
    scanMode = false;
    connect(this, SIGNAL(showStrength(float)), mr, SLOT(showStrength(float)));
...
void dabProcessor::run() {
    ...
    notSynced:
        ...
        myReader.getSamples(ofdmBuffer.data(), T_u, coarseOffset + fineOffset);
        /*
    ...
    Check_endofNULL:
        ...
        myReader.getSamples(ofdmBuffer.data(), T_u, coarseOffset + fineOffset);
        /*
         *	We now have to find the exact first sample of the non-null
    ...
    SyncOnPhase:
         ...
         */
        memmove(ofdmBuffer.data(), &((ofdmBuffer.data())[startIndex]),
                (T_u - startIndex) * sizeof(std::complex<float>));
        int ofdmBufferIndex = T_u - startIndex;

        /*
	 * Block_0:
         *	Block 0 is special in that it is used for fine time
         *	synchronization, for coarse frequency synchronization and its content
        ...
        /*
         *	after block 0, we will just read in the other (params -> L - 1)
         *	blocks
	 *
         * Data_blocks:
        ...
        for (int ofdmSymbolCount = 1; ofdmSymbolCount < 4; ofdmSymbolCount++) {
            myReader.getSamples(ofdmBuffer.data(), T_s,
        ...
        for (int ofdmSymbolCount = 4; ofdmSymbolCount < nrBlocks;
             ofdmSymbolCount++) {
            myReader.getSamples(ofdmBuffer.data(), T_s,
                                coarseOffset + fineOffset);
            sampleCount += T_s;
         ...
         */
        myReader.getSamples(ofdmBuffer.data(), T_null,
                            coarseOffset + fineOffset);
        sampleCount += T_null;
        float sum = 0;
        for (i = 0; i < T_null; i++)
        ...
        static float snr = 0;
        snr =
            0.9 * snr + 0.1 * 20 * log10((myReader.get_sLevel() + 0.005) / sum);
        static int ccc = 0;
        if (++ccc >= 2) {
            ccc = 0;

            // FIXME consistent strength measure
            showStrength((getDb(myReader.get_sLevel(), 128) - getDb(0, 128)) *
                         100);
        }
        /*
...
void dabProcessor::getFrameQuality(int *totalFrames, int *goodFrames,
                                   int *badFrames) {
...
void dabProcessor::getParameters(const QString &s, uint32_t *p_SId,
                                 int *p_SCIds) {
...
void dabProcessor::dataforPacketService(const QString &s, packetdata *pd,
                                        int16_t compnr) {
    my_ficHandler.dataforPacketService(s, pd, compnr);
}

uint8_t dabProcessor::get_ecc() { return my_ficHandler.get_ecc(); }
...
void dabProcessor::set_epgData(int SId, int32_t theTime, const QString &s) {
...
void dabProcessor::startDumping(SNDFILE *f) { myReader.startDumping(f); }

void dabProcessor::stopDumping() { myReader.stopDumping(); }
