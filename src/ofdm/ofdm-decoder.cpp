...
void ofdmDecoder::processBlock_0(std::vector<std::complex<float>> buffer) {
    memcpy(fft_buffer, buffer.data(), T_u * sizeof(std::complex<float>));
...
float ofdmDecoder::computeQuality(std::complex<float> *v) {
    ...
    for (i = 0; i < carriers; i++) {
        x[i] = std::complex<float>(abs(real(v[T_u / 2 - carriers / 2 + i])),
                                   abs(imag(v[T_u / 2 - carriers / 2 + i])));
        avgPoint += x[i];
    }
    ...
    for (i = 0; i < carriers; i++) {
        ...
        denominator +=
            square(absVal - abs(real(x[i]))) + square(absVal - abs(imag(x[i])));
    }
...
void ofdmDecoder::compute_timeOffset(std::complex<float> *r,
    ...
    for (int i = -carriers / 2; i < carriers / 2; i += 6) {
        ...
        leftTerm =
            s * conj(std::complex<float>(abs(s) / sqrt(2), abs(s) / sqrt(2)));
...
void ofdmDecoder::compute_clockOffset(std::complex<float> *r,
    ...
    for (int i = -carriers / 2; i < carriers / 2; i += 6) {
        ...
        std::complex<float> a1 =
            std::complex<float>(abs(real(r[index])), abs(imag(r[index])));
        std::complex<float> a2 =
            std::complex<float>(abs(real(v[index])), abs(imag(v[index])));
