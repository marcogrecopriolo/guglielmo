...
static DSPFLOAT sinhm1(DSPFLOAT x) {
    return log(x + sqrt(x * x + 1));
}

static DSPFLOAT coshm1(DSPFLOAT x) {
    return log(x + sqrt(x * x - 1));
}
...
static DSPFLOAT Bilineair(element* Transfer, int fs, int NumofQuad) {
    element* CurrentQuad;
...
LowPassIIR::LowPassIIR(int16_t apass, int16_t astop,
    int32_t fpass, int32_t fstop, int32_t fs): Basic_IIR((guessOrderB(apass, astop,
                     fpass, fstop) + 1) / 2) {
